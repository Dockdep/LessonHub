...
| `backend/04-infrastructure.md` | `LessonsHub.Infrastructure/Data/*.cs`, `LessonsHub.Infrastructure/Repositories/*.cs`, `LessonsHub.Infrastructure/Services/*.cs`, `LessonsHub.Infrastructure/Auth/*.cs`, `LessonsHub/Program.cs` (HTTP pipeline + resilience handler config) |
...
- [ ] No mention of removed concepts: `Context7Backend`, `KNOWN_FRAMEWORK_DOC_SITES`, `detect_primary_framework`, `extract_technologies`, `IUnitOfWork`, `MemPalace`.
